...
def test_run_single(n_components=60, undersampling=0.7, train_amount=0.8, batch_size=1000, n_iter=250, fit_algorithm='lars', alpha_train=0.2, transform_algorithm='omp', verbose=0):   
    ...
    plt.subplot(1,4,4)
    plt.imshow((imgs_test_ref[:,:,0,0]-recs[:,:,0])**2,cmap='gist_heat')
    plt.title('MSE')
    
    # Slice through time
    plt.figure(figsize=(10,10))
    plt.subplot(1,4,1)
    plt.imshow(imgs_test_ref[:,int(imgs_test_ref.shape[1]/2),:,0],cmap='gray')
    plt.title("Ground")
    plt.xlabel('Frames')
    plt.ylabel('y')
    plt.subplot(1,4,2)
    plt.imshow(imgs_test[:,int(imgs_test.shape[1]/2),:,0],cmap='gray')
    plt.title("Aliased")
    plt.xlabel('Frames')
    plt.ylabel('y')
    plt.subplot(1,4,3)
    plt.imshow(recs[:,int(recs.shape[1]/2),:],cmap='gray')
    plt.title("Reconstruction")
    plt.xlabel('Frames')
    plt.ylabel('y')
    plt.subplot(1,4,4)
    plt.imshow((imgs_test_ref[:,int(imgs_test_ref.shape[1]/2),:,0]-recs[:,int(recs.shape[1]/2),:])**2,cmap='gist_heat')
    plt.title('MSE')
    plt.xlabel('Frames')
    plt.ylabel('y')
    # Show Dynamic Plots
    plt.figure(figsize=(10,10))
    plt.plot(imgs_test_ref[int(imgs_test_ref.shape[1]/2),:,0,0], label='reference', color='b')
    plt.plot(imgs_test[int(imgs_test.shape[1]/2),:,0,0], label='test', color='y')
    plt.plot(recs[int(recs.shape[1]/2),:,0], label='recs', color='c')
    plt.plot((imgs_test_ref[int(imgs_test_ref.shape[1]/2),:,0,0]-recs[int(recs.shape[1]/2),:,0])**2,label='MSE', color='r')
    plt.legend()
    plt.xlabel('x')
